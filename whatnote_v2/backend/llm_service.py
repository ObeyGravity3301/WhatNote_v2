            ...
            for iteration in range(max_iterations):
                ...
                if finish_reason == 'tool_calls' and message.get('tool_calls'):
                    ...
                    if function_name in [
                        ...
                        # 详细日志：显示所有 todo 项
                        if current_status and current_status.get('items'):
                            items_preview = [f"{i}: {item.get('task', '?')[:20]}" for i, item in enumerate(current_status.get('items', []))]
                            info(f"[LLM Tools][Todo] 发送 todo_status 事件: total={current_status.get('total')}, items={items_preview}")
                        else:
